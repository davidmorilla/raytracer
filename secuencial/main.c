...
int generate_random_spheres(Object *spheres, int num_spheres, vec3 box_min, vec3 box_max)
{
    ...
    const double min_radius = 1.5, max_radius = 5.0;
    int seed= time(NULL);
    int iterations = 0;
    int spheres_found = 0;
...
int main(int argc, char **argv)
{
    ...
    printf("seed = %d\n", seed);
    srand(seed);
    Object* spheres = malloc(sizeof(Object)*20);
    int num_spheres = generate_random_spheres(spheres, 20, VECTOR(-30,-18,15), VECTOR(30,18, 27));
 ...
 /* light */
        {
            .color = WHITE, 
            .flags = M_DEFAULT,
            .emission = RGB(0x00 * 15, 0x32 * 15 -100, 0xA0 * 15 -300),
